...
TESTER_DIR = (
    "tools"  # Windows用のコンパイル済みバイナリの場合は "tools_x86_64-pc-windows-gnu"
)
#TESTER_CMD = "./tools/target/release/vis"  # cargo runより直接呼んだ方が速い ※cargo build --release --bin vis で事前にビルド必要
...
TEST_IDS = list(
    range(START_SEED, START_SEED + SIZE * STEP, STEP)
)  # テスト対象seed番号リスト生成
...
class Task:
    @classmethod
    def execute(cls, input_file: str = "", output_file: str = "", seed: int = 0):  # type: ignore
        ...
        # スコア計算
        cmd_ = "cargo run -r --bin vis"
        execute_tester_command = f"{cmd_} {input_file} {output_file}"
        
        
        #print(execute_tester_command)
        result = subprocess.run(
        ...
        std_err = result.stderr.decode("utf-8", "ignore")
        for out in std_err.split("\n"):
            #print(out)
